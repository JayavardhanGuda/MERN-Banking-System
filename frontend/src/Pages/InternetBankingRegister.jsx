...
import {
  ...
  FaKey, FaInfoCircle, FaSpinner
} from 'react-icons/fa';
import { Link, useNavigate } from 'react-router-dom';
import { registerInternetBanking, checkInternetBankingStatus } from '../services/api';
...
export default function InternetBankingRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password === formData.transactionPassword) {
      setErrors({ transactionPassword: 'Login password and transaction password must be different' });
      return;
    }
    ...
    try {
      const response = await registerInternetBanking({
      ...
      if (response.success) {
        setSuccessMessage('Internet Banking registered successfully! Redirecting to dashboard…');
        setIsRegistered(true);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      // For update, we'd need an update API endpoint
          ...
          <div className="ibr-body">
            ...
            >
              ...
              <div className="ibr-form-actions">
                ...
                <button type="submit" className="ibr-btn ibr-btn--primary" disabled={submitting}>
                  {submitting ? (
