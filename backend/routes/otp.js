...
// Mock SMS sending (In production, use Twilio, AWS SNS, or other SMS providers)
...
router.post('/verify-email', async (req, res) => {
  try {
    ...
    // Return masked contact info for OTP delivery
...
// Step 2: Generate and send OTP for forgot password
router.post('/forgot-password/send', async (req, res) => {
  try {
    const { email, accountNumber } = req.body;

    if (!email && !accountNumber) {
      return res.status(400).json({ success: false, message: 'Email or account number is required' });
    }
    ...
    // Determine message based on delivery status
    let deliveryMessage = '';
...
router.post('/forgot-password/verify', async (req, res) => {
  try {
    ...
    if (!accountNumber || !otp) {
      return res.status(400).json({ success: false, message: 'Account number and OTP are required' });
    }
    ...
    // Store reset token in user document (or you can use a separate collection)
...
// Get OTPs by account number (for admin/debugging)
router.get('/account/:accountNumber', async (req, res) => {
  try {
    const otps = await Otp.find({ accountNumber: req.params.accountNumber }).sort({ createdAt: -1 });
...
router.post('/registration/send', async (req, res) => {
  try {
    const { email } = req.body;
...
router.post('/registration/verify', async (req, res) => {
  try {
    ...
    // Debug: Find all OTPs for this email
    const allOtps = await Otp.find({ email: searchEmail, type: 'registration' }).sort({ createdAt: -1 }).limit(5);
